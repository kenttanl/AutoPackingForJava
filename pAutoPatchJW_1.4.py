...
# 允许的文件扩展名
FILTER_EXTEN = ['.java', '.class', '.xml', '.properties', '.jsp', '.js', '.html', '.css', '.tld']
...
class Patch():
    ...
    def parseSvnPatchFile(self):
        ...
        global FILTER_EXTEN
        global IS_WEB_PROJECT
        global IS_MAVEN_PROJECT
        ...
        for line in patchFile.readlines():
            ...
            if prefixIden == 'Index:':
                ...
                if len(lineList) >= 2:
                    ...
                    # 将java文件替换成class文件
                    classDir = 'WebRoot/WEB-INF/classes/'
                    if int(IS_WEB_PROJECT) != 1:
                        classDir = 'target/classes/'

                    # 支持Maven项目
                    if int(IS_MAVEN_PROJECT) == 1:
                        possibleFilename = possibleFilename.replace('src/main/java', classDir) 
                        possibleFilename = possibleFilename.replace('src/main/resources', classDir)

                    possibleFilename = possibleFilename.replace('src/', classDir)
    ...
    def generatePatch(self):
        ...
        self.patchLog.append('')
        self.patchLog.append('  >> 尝试拷贝：' + str(successCount + failCount) + '，成功：' + str(successCount) + '，失败：' + str(failCount))
        self.patchLog.append('')
    ...
    def printPatchFile(self):
        ...
        for filename in self.patchFileList:
            # print('  >>', filename)
            indexStr = (' [' + str(index) + ']').rjust(5)
            self.patchLog.append('  >>' + indexStr + ' ' + filename)
            index = index + 1
        # print("--------------------------------------------------------------------------------")
            ...
            if len(line) > 0:
                ...
                if '=' in line:
                    ...
                    if len(attrMap) >= 2:
                        ...
                        elif key == 'IS_WEB_PROJECT':
                            IS_WEB_PROJECT = value
                        elif key == 'IS_MAVEN_PROJECT':
                            IS_MAVEN_PROJECT = value
            configFile.close()
        # pass
