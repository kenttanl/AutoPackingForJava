...
class Patch():
        ...
        for line in patchFile.readlines():
            ...
            if prefixIden == 'Index:':
                ...
                if len(lineList) >= 2:
                    ...
                    # 新增xml配置文件路径转换（金网定制版），20160329
                    possibleFilename = possibleFilename.replace('WebRoot/WEB-INF/classes/gnnt/MEBS/issue/mgr/configfiles/', 'WebRoot/WEB-INF/classes/')
    ...
    def generatePatch(self):
        ...
        tomcatDir = "";
        for key in PROJECT_AND_TOMCAT:
            if self.projectName in PROJECT_AND_TOMCAT[key]:
                tomcatDir = key + "/" + "webapps"
        
        # 补丁生产目录添加上补丁名称与项目名称
        if (PATCH_GENERATE_PATH == ''):
            patchDir = PATCH_NAME + tomcatDir + "/" + self.projectName + "/"
        else:
            patchDir = PATCH_NAME + PATCH_GENERATE_PATH + '/' + tomcatDir + "/" + self.projectName + "/"
            ...
            if len(line) > 0:
                ...
                if '=' in line:
                    ...
                    if len(attrMap) >= 2:
                        ...
                        elif key == 'PROJECT_AND_TIMCAT_TABLE':
                            # 读取项目与TOMCAT对照表
                            ptMap = value.split(':')
                            PROJECT_AND_TOMCAT[ptMap[0]] = ptMap[1]
